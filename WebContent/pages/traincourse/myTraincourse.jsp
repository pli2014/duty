  ...
  function receive(){
    ...
    if(selectedRows.length == 0){
      ...
      if (confirm("您确定要接受 "+ selectedRows.length +" 培训课程吗?")){
	      ...
	      window.location.href = "frontTraincourse/receive.action?" + par;
      }
    }
}
	...
	<div class="home2">
		...
		<div class="train-hr">
			<div class="train-hr-left">
				<s:if test="start > 0">
					<a class="pre"
						href="frontTraincourse/index.action?start=${start-length}">上一页</a>
				</s:if>
				<s:else>
				...
				<s:if test="(start + length) < count">
					<a class="pre"
						href="frontTraincourse/index.action?start=${start+length}">下一页</a>
				</s:if>
				<s:else>
